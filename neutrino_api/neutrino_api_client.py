...
class NeutrinoAPIClient:
    ...
    def exec_request(
        ...
        headers = {"User-ID": self.user_id, "API-Key": self.api_key}
        try:
            kwargs = {
                "headers": headers,
                "stream": True,
            ...
            if status_code == 200:
                ...
                if "application/json" in content_type:
                    return APIResponse.of_data(status_code, content_type, response.json())
                elif output_file_path is not None:
                    with open(output_file_path, "wb") as file_pointer:
                        for buf in response.iter_content(chunk_size=2048):
                            if not buf:
                                break
                            file_pointer.write(buf)
                    return APIResponse.of_output_file_path(
                        status_code,
                        content_type,
                        os.path.abspath(output_file_path)
                    )
                else:
                    return APIResponse.of_http_status(status_code, content_type, APIErrorCode.API_GATEWAY_ERROR, response.text)
            else:
                # Non-200 error received
                if "application/json" in content_type:
                    json = response.json()
                    api_error = json.get("api-error")
                    api_error_msg = json.get("api-error-msg")
                    ...
                    return APIResponse.of_http_status(
                        status_code, content_type, APIErrorCode.NETWORK_IO_ERROR, response.text
                    )
            ...
            return APIResponse.of_cause(APIErrorCode.URL_PARSING_ERROR, err)
            ...
            return APIResponse.of_cause(APIErrorCode.TIMEOUT, err)
